...
class ClaudeOpus4(Model):
    def __init__(self):
        self.ClaudeOpus4_Init()

    def Run(self, file, inp, start_time, end_time, question_time, omni=False, proactive=False, salience_map_path=None):
        return self.ClaudeOpus4_Run(file, inp, start_time, end_time, question_time, omni, proactive, salience_map_path)
    
    def name(self):
        return "Claude-Opus-4"

    def ClaudeOpus4_Init(self):
        """Initialize AWS Bedrock client for Claude Opus 4"""
        print("🔧 Initializing Claude Opus 4 with AWS Bedrock...")
        
        self.client = boto3.client('bedrock-runtime', region_name='us-east-2')
        self.model_id = 'us.anthropic.claude-opus-4-20250514-v1:0'
        
        print("✅ Claude Opus 4 client initialized successfully!")
    ...
    def ClaudeOpus4_Run(self, file, inp, start_time, end_time, question_time, omni=False, proactive=False, salience_map_path=None):
        """Run Claude Opus 4 inference on video segment"""
        try:
            T_start = time.time()
            ...
            print(f"❌ Error in Claude Opus 4 inference: {e}")
            import traceback
            traceback.print_exc()
            return " ", 0
