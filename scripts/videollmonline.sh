#!/bin/bash
# VideollmOnline evaluation script for StreamingGaze
...
VIDEOLLM_ONLINE_DIR="${VIDEOLLM_ONLINE_PATH:-path/to/videollm-online}"
export PYTHONPATH="$ROOT_DIR:$VIDEOLLM_ONLINE_DIR"
export DECORD_EOF_RETRY_MAX=40960  # Increase EOF retry limit for problematic videos
# ===== MODEL CONFIGURATION =====
MODEL_NAME="VideollmOnline_Streaming"
EVAL_NAME="VideollmOnline-8B"
MODEL_PATH="${VIDEOLLM_ONLINE_MODEL_PATH:-path/to/videollm-online-model}"
export VIDEOLLM_ONLINE_MODEL_PATH="$MODEL_PATH"  # Pass model path to Python code
# ===============================
WORKDIR="$ROOT_DIR/results/VideollmOnline_Streaming"
...
PIDS=()
GPU_ARRAY=(0 1 2 3)  # Adjust GPUs as needed
GPU_IDX=0
echo ""
...
echo "========================================="
if [ $FAILED -eq 0 ]; then
    echo "✓ All VideollmOnline evaluations completed successfully!"
