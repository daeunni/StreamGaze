...
# source /home/colligo/miniconda3/bin/activate vi2
...
# ===== CHANGE MODEL HERE =====
# For LoRA finetuned models, specify BOTH base model and LoRA path:

# For pretrained models (no LoRA), just set BASE_MODEL:
BASE_MODEL="${VISPEAK_MODEL_PATH:-path/to/vispeak/model}"
LORA_PATH=""
# ======================================

# Set ViSpeak model paths
export VISPEAK_MODEL_PATH="$BASE_MODEL"
export VISPEAK_LORA_PATH="$LORA_PATH"

# For result directory naming
if [ -n "$LORA_PATH" ]; then
    EVAL_NAME=$(basename "$LORA_PATH")
else
    EVAL_NAME=$(basename "$BASE_MODEL")
fi

MODEL_NAME="ViSpeak"  # Must be "ViSpeak" for eval.py
WORKDIR="$ROOT_DIR/results/ViSpeak"
...
mkdir -p "$LOGDIR"
mkdir -p "$RESULTS_DIR"
echo "🔧 Using ViSpeak base model: $VISPEAK_MODEL_PATH"
if [ -n "$VISPEAK_LORA_PATH" ]; then
    echo "🔧 Using LoRA weights: $VISPEAK_LORA_PATH"
fi
...
PIDS=()
# GPU_ARRAY=(4 5 6 7)  # Use GPUs 4, 5, 6, 7
GPU_ARRAY=(0 1 2 3)
GPU_IDX=0
